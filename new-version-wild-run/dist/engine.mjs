...
export function freshState(){return {mode:'menu',distance:0,points:0,hearts:3,energy:100,lane:0,x:0,y:0,vy:0,speed:18,carrot:0,gold:0,mud:0,sting:0,invincible:0,duck:0,jumpBuffer:0,sprinting:false,combo:0,bestCombo:0,time:0};}
...
export function tick(s,dt){
  ...
  const sprint=s.sprinting&&s.energy>1&&!s.mud&&!s.sting;
  s.energy=Math.max(0,Math.min(100,s.energy+(sprint?-34:s.sprinting?0:13)*dt));
  let target=(18+Math.min(10,s.distance/250))*(s.gold?1.75:s.carrot?1.3:1)*(sprint?1.32:1)*(s.mud?.52:1)*(s.sting?.7:1);
...
export function interact(s,item){
  ...
  if(['carrot','apple','gold'].includes(type)){
    ...
    if(type==='carrot'){s.carrot=4;s.energy=Math.min(100,s.energy+30);return 'carrot';}
    if(type==='apple'){s.hearts=Math.min(3,s.hearts+1);s.energy=Math.min(100,s.energy+20);s.invincible=Math.max(s.invincible,2);return 'apple';}
    s.gold=5;s.mud=0;s.sting=0;return 'gold';
  }
...
export function makeRow(distance,rng=Math.random){
  ...
  for(let lane=-1;lane<=1;lane++){
    if(lane===safe){const r=rng();result.push({lane,type:r<.08?'gold':r<.24?'apple':'carrot'});}
